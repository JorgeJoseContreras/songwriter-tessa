...
let quill: any = null; // To hold the Quill editor instance

// --- SONGWRITER ELEMENTS ---
const songForm = document.getElementById('song-form');
const songPromptInput = document.getElementById('song-prompt') as HTMLTextAreaElement;
const generateBtn = document.getElementById('generate-song-btn') as HTMLButtonElement;
...
const submitSongBtn = document.getElementById('submit-song-btn') as HTMLButtonElement;
...
const openEmailClientBtn = document.getElementById('open-email-client-btn') as HTMLAnchorElement;
...
// --- SONGWRITER LOGIC ---
const handleSongGeneration = async (e: Event) => {
    e.preventDefault();
    ...
    const selectedInstruments = Array.from(document.querySelectorAll('.instrument-btn.selected'))
        .map(btn => (btn as HTMLElement).dataset.instrument);
...
const handleSubmitToLabel = () => {
    ...
    if (emailSubjectDisplay) emailSubjectDisplay.textContent = subject;
    if (emailBodyDisplay) emailBodyDisplay.textContent = body;

    // Create mailto link
    const mailtoLink = `mailto:aghlc.nm@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    if (openEmailClientBtn) openEmailClientBtn.href = mailtoLink;
...
const handleCopy = async (e: Event) => {
    ...
    if (!elementToCopy || !elementToCopy.textContent) return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (songLyricsEditor) {
        ...
        quill = new (window as any).Quill('#song-lyrics-editor', {
            modules: {
